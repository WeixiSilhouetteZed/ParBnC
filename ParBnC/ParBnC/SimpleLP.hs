...
feasibleSolutionFound :: LA.Matrix R -> Bool
feasibleSolutionFound tab = (roundSolution lastVal) == 0 where
    lastRow = last $ LA.toRows tab
    lastVal = lastRow ! (LA.cols tab - 1)
...
simplexWithMixedTab obj tab
    ...
    | otherwise  = (lastVal, solution, lastTab) where
        ...
        interTab
            | needed = updateMixedTab phaseOneTab 100
        ...
        lastTab = updateTab obj phaseTwoTab 100
...
constructParBranchAndBound :: ObjectiveType -> Matrix R -> Vector Bool -> Vector R -> Int -> Tree BranchProblem
constructParBranchAndBound obj tab intMask costVec maxDepth
    | infeasible = Nil
    | maxDepth == 0 = Nil
    | and $ integerSolved intList solMask = currProb Nil Nil
    | otherwise = currProb leftTree rightTree where
...
constructParBranchAndCut :: ObjectiveType -> Matrix R -> Vector Bool -> Vector R -> Int -> Tree BranchProblem
constructParBranchAndCut obj tab intMask costVec maxDepth
    | infeasible = Nil
    | maxDepth == 0 = Nil
    | and $ integerSolved intList solMask = currProb Nil Nil
    | otherwise = currProb leftTree rightTree where
...
searchBBTreeMax :: Tree BranchProblem -> Vector Bool -> BranchProblem
searchBBTreeMax Nil _ = BranchProblem{solution = LA.fromList [], value = -infinity}
searchBBTreeMax (Node bp Nil Nil) intMask
    | and $ integerSolved intList solMask = bp
    | otherwise = BranchProblem{solution = LA.fromList [], value = -infinity} where
        intList = LA.toList intMask 
        solMask =  map (isInt . roundSolution) $ LA.toList $ solution bp
searchBBTreeMax (Node bp leftBpNode Nil) intMask = searchBBTreeMax leftBpNode intMask
searchBBTreeMax (Node bp Nil rightBpNode) intMask = searchBBTreeMax rightBpNode intMask
searchBBTreeMax (Node bp leftBpNode rightBpNode) intMask
    | leftBpVal > rightBpVal = leftBp
    | otherwise = rightBp where
        leftBp = searchBBTreeMax leftBpNode intMask
        rightBp = searchBBTreeMax rightBpNode intMask
        midBpVal = value bp
...
branchAndBound obj tab intMask costVec = searchBBTreeMax (constructBranchAndBound obj tab intMask costVec 10) intMask 

branchAndCut :: ObjectiveType -> Matrix R -> Vector Bool -> Vector R -> BranchProblem
branchAndCut obj tab intMask costVec = searchBBTreeMax (constructBranchAndCut obj tab intMask costVec 10) intMask
...
testTab :: LA.Matrix R
testTab = addSlackMatrix $ toTableau testC testMat testB
